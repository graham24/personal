...
class personalProjectList extends React.Component {
    ...
    renderList() {
        ...
        return this.props.personalProjects.map((personalProject) => {
            return (
                <div key={personalProject.id} onClick={() => this.selectProject(personalProject)} className={"mb-4 hover:bg-gray-50 hover:bg-opacity-5 rounded-md hover:cursor-pointer" + (personalProject.id === activeProject ? ' bg-gray-50 bg-opacity-5' : '')}>
...
export default connect(mapStateToProps, { fetchPersonalProjects, fetchProjectREADME })(personalProjectList);